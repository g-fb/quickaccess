    ...
    : QWidget(parent)
{
    ...
    iconPreview->setIcon(QIcon::fromTheme(u"unknown"_s));
    ...
    connect(separatorActioon, &QAction::triggered, this, [=]() {
        ...
        item->setData(Qt::UserRole, u"separator"_s);
        m_foldersListView->addItem(item);
    ...
    connect(clipboardAction, &QAction::triggered, this, [=]() {
        ...
        if (fileInfo.exists()) {
            auto *item = new QListWidgetItem(QIcon::fromTheme(u"folder"_s), clipboard->text());
            item->setToolTip(i18n("Double click to set icon"));
            m_foldersListView->addItem(item);
            m_foldersListView->setCurrentItem(item);
            Q_EMIT changed();
    ...
    deleteFolderButton->setIcon(QIcon::fromTheme(u"edit-delete"_s));
    ...
    connect(addFolderButton, &QToolButton::clicked, this, [=]() {
        ...
        if (!path.isEmpty()) {
            auto *item = new QListWidgetItem(QIcon::fromTheme(u"folder"_s), path);
            item->setToolTip(i18n("Double click to set icon"));
            m_foldersListView->addItem(item);
            m_foldersListView->setCurrentItem(item);
            Q_EMIT changed();
...
void FoldersSettingsPage::save()
{
    ...
    for (int i = 0; i < foldersCount; ++i) {
        ...
        auto group = m_config->group(QString(u"Folder_%1"_s).arg(i));
...
{
    int foldersCount = m_config->group(u"General"_s).readEntry("FoldersCount").toInt();
    // delete all folders
    for (int i = 0; i < foldersCount; ++i) {
        auto group = m_config->group(QString(u"Folder_%0"_s).arg(i));
        m_config->deleteGroup(group.name());
    }
    m_config->deleteGroup(u"Folders"_s);
    m_config->sync();
}
